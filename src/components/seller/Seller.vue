        ...
        if(response.code == 0){
          ...
          this.$nextTick(()=>{
            if(this.seller.poi_env.thumbnails_url_list){
              let imgW = this.$refs.picsItems[0].clientWidth
              let marginR = 11
...
.seller .seller-wrapper .other-wrapper .sever-wrapper{
...
.seller .seller-wrapper .other-wrapper .sever-wrapper .poi_server{
  display: inline-block;
  margin-left: 17px;
}
.seller .seller-wrapper .other-wrapper .sever-wrapper .poi_server img {
		width: 15px;
		height: 15px;
		margin-right: 6px;
...
.seller .seller-wrapper .other-wrapper .discouts-wrapper .discounts-items .text{
  flex: 1;
  font-size: 14px;
}
</style>
